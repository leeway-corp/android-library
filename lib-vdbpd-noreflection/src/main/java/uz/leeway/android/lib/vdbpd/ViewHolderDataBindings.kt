@file:Suppress("unused")
@file:JvmName("ViewHolderBindings")

package uz.leeway.android.lib.vdbpd

import android.view.View
import androidx.annotation.IdRes
import androidx.core.view.ViewCompat
import androidx.databinding.ViewDataBinding
import androidx.recyclerview.widget.RecyclerView.ViewHolder

/**
 * Create new [ViewDataBinding] associated with the [ViewHolder]
 */
fun <VH : ViewHolder, T : ViewDataBinding> VH.viewDataBinding(viewBinder: (VH) -> T): ViewDataBindingProperty<VH, T> {
    return LazyViewDataBindingProperty(viewBinder)
}

/**
 * Create new [ViewDataBinding] associated with the [ViewHolder]
 *
 * @param vbFactory Function that create new instance of [ViewDataBinding]. `MyViewBinding::bind` can be used
 * @param viewProvider Provide a [View] from the [ViewHolder]. By default call [ViewHolder.itemView]
 */
inline fun <VH : ViewHolder, T : ViewDataBinding> VH.viewDataBinding(
    crossinline vbFactory: (View) -> T,
    crossinline viewProvider: (VH) -> View = ViewHolder::itemView,
): ViewDataBindingProperty<VH, T> {
    return LazyViewDataBindingProperty { viewHolder: VH -> viewProvider(viewHolder).let(vbFactory) }
}

/**
 * Create new [ViewDataBinding] associated with the [ViewHolder]
 *
 * @param vbFactory Function that create new instance of [ViewDataBinding]. `MyViewBinding::bind` can be used
 * @param viewBindingRootId Root view's id that will be used as root for the view binding
 */
inline fun <VH : ViewHolder, T : ViewDataBinding> VH.viewDataBinding(
    crossinline vbFactory: (View) -> T,
    @IdRes viewBindingRootId: Int,
): ViewDataBindingProperty<VH, T> {
    return LazyViewDataBindingProperty { viewHolder: VH ->
        ViewCompat.requireViewById<View>(viewHolder.itemView, viewBindingRootId).let(vbFactory)
    }
}
